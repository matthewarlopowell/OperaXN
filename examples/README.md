...
- **`inhouse/`** — three laboratory XRD scans (`.dat`, collected 10:05–10:15
...
2. In the upload dialog, select the data source (laboratory / in-house for
   `inhouse/`, neutron for `neutron/`), point it at the generated folder and
   load with **absolute** time correlation.
3. Each diffraction scan is now annotated with the electrochemical state of
   the cell at acquisition time — the in-house scan at 10:05 correlates to
...
These entries are placeholders: no public datasets are distributed yet, and
the tests' expected values are calibrated to the authors' local reference
data. Any subset may be present; missing datasets are reported as skips.
To contribute a public example dataset, add it under this layout and add a
test to the real-data section of `tests/test_pipeline.py` describing what
it should produce — see [../CONTRIBUTING.md](../CONTRIBUTING.md).
